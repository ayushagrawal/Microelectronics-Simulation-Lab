...
for j=1:size(work_function,1)
    ...
    for i=1:length
        ...
        thickness = 10*nm;
        V_applied(i) = Vs(i) + E_sio2*thickness + work_function(j);
...
%xlim([-2 3]);
%ylim([0 1.2]);
legend('\phi_F = -0.6eV','\phi_F = -0.3eV','\phi_F = 0eV','\phi_F = 0.3eV','\phi_F = 0.6eV');